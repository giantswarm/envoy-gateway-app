...
def render_markdown(data, report_url=None):
    ...
    if report_url:
        lines += ["", f"📦 [Download the full report with charts]({report_url}) "
                      "— then extract and open it locally:",
                  "", "```bash", "tar -xzf report.tar.gz && open report.html",
                  "```"]
...
def main():
    ...
    ap.add_argument("--report-url", default=None,
                    help="download URL of the report.tar.gz, linked from the "
                         "markdown summary")
    args = ap.parse_args()
