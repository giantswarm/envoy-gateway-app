...
func buildTestRunUnstructured(name, namespace, configMapName, baseDomain, testID string) *unstructured.Unstructured {
	...
	env := []any{
		...
		// Basic-auth credentials the scenario sends on the "valid credentials"
		// request; must match what enforceBasicAuth* provisions on the gateways.
		map[string]any{"name": "BASIC_AUTH_USER", "value": basicAuthUser()},
		map[string]any{"name": "BASIC_AUTH_PASSWORD", "value": basicAuthPassword()},
	}
