...
func TestPerformance(t *testing.T) {
	suite.New().
		...
		Tests(func() {
			...
			// With the unauthenticated baseline confirmed above, enforce HTTP
			// Basic Auth at each gateway and verify it before load testing the
			// authenticated path. The k6 basic-auth scenario then asserts
			// valid->200, wrong->401, missing->401 under load.
			It("should enforce basic auth on the envoy gateway", FlakeAttempts(3), func() {
				enforceEnvoyBasicAuth()
			})
			if proxyController == proxyControllerNginx {
				It("should enforce basic auth on ingress-nginx", FlakeAttempts(3), func() {
					enforceNginxBasicAuth()
				})
			}
			if proxyController == proxyControllerKong {
				It("should enforce basic auth on kong", FlakeAttempts(3), func() {
					enforceKongBasicAuth()
				})
			}
			It("should require basic auth on envoy gateway", func() {
				DeferCleanup(func() {
					if CurrentSpecReport().Failed() {
						AbortSuite("envoy gateway basic auth not enforced, aborting remaining tests")
					}
				})
				expectEndpointRequiresAuth(envoyUrl)
			})
			if proxyController == proxyControllerNginx {
				It("should require basic auth on ingress-nginx", func() {
					DeferCleanup(func() {
						if CurrentSpecReport().Failed() {
							AbortSuite("ingress-nginx basic auth not enforced, aborting remaining tests")
						}
					})
					expectEndpointRequiresAuth(nginxUrl)
				})
			}
			if proxyController == proxyControllerKong {
				It("should require basic auth on kong", func() {
					DeferCleanup(func() {
						if CurrentSpecReport().Failed() {
							AbortSuite("kong basic auth not enforced, aborting remaining tests")
						}
					})
					expectEndpointRequiresAuth(kongUrl)
				})
			}
		...
		Run(t, "Basic Auth Performance Test")
}
